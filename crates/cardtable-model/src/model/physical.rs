//! The **physical model** — the conserved pile/card types and their behaviors: the source-of-truth layer
//! (`Card` / `Pile` / `Board`), sibling to the `ui` (attention) and `geometry` submodules under `model`.
//! Dependency-free: no `contract`, no Bevy.
//!
