...
use rules::combat::step_game::{
    STEPS as SCHEDULE, Step, StepChoice, StepCombat, StepScorer, StepState, step_coord, step_policy,
};
...
/// **The round, step by step** - re-runs the deterministic resolution on a throwaway clone of the pre-round
/// board (identical to what `self.state` just resolved) and hands the transcript to the SHARED formatter
/// ([`rules::combat::narrate`]), so this log and the card-table arena's journal can never tell the story of a
...
fn narrate_steps(before: &Board, script: &StepScript) -> Vec<String> {
    ...
    rules::combat::narrate::narrate(before, &transcript)
}
