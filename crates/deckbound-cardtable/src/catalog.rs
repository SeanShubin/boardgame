//! The single source of truth for the card-table world's **content** — the five stats, the abilities, the
//! starter roster, and the map creatures/encounters — as Rust consts (not a data file). The emitter reads
...
/// `[Might, Vitality, Toughness, Cadence, Finesse]` — the same order a kit's stat line uses, so values
