...
//! an sub-phase stops cycling**. Swapping a human / scripted AI / solver swaps *this* module; the
//! mechanics in [`crate::combat`] do not change.
//!
//! This is the live mirror of the validated `sub_phase.rs` sim's policy half (`priorities` / `step_of`
...
//!   `V→O→R`. A unit holds its Tempo for a higher-priority sub-phase still to come rather than spend
...
/// any non-Rearguard target this is always true. Mirrors the gate in `sub_phase::choose_target`.
pub fn can_reach(
...
/// `sub_phase::choose_target`. Returns the chosen enemy's index in `def`.
...
/// **The governing target for `attacker` this sub-phase step**, by the role priority list timed
/// against the schedule (§4.6). Walk the priorities in order; the first one with a crackable target is
/// the goal. Strike it **iff its sub-phase is the current `step_idx`**; **hold** (return `None`,
/// committing nothing) if its window is still to come; **skip** to the next priority once its window
/// has passed. Mirrors the inner targeting loop of `sub_phase::run_round_logged`. Returns the chosen
...
/// exceed `Fa`, so the minimum cards is `floor(Fa / Fd) + 1`. Mirrors `sub_phase::avoid_cost`.
...
/// rule. Mirrors `sub_phase::default_hits`. Returns `true` if the role evades.
...
/// only if it can afford the bid. Mirrors `sub_phase::should_avoid`. (A *group* never slips — that is
...
/// the reflexive strike-back gate in `sub_phase::run_round_logged` (which counts the attacker's banked
