...
Winning a read **banks advantage** as Power / Speed / Precision cards in your
...
and Scheme, convert it with a Strike or an extra turn — but every read risks the whole
...
- Do **special tactic cards** (feints, counters, mind-games) modify the basic cycle,
  and how do they enter/leave play relative to the self-returning core reads?
- Does a won **Defense** ever land a **counter-hit**, or only negate + bank (+ a Speed
  extra action)?
- How **mitigation** Form cards phrase "keep some bank on a misread."
