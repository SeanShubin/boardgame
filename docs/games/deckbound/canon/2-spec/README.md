...
| **The strike** (tactical core)                           | ✅ worked    | **[`combat.md`](combat.md)** — the deterministic **Interaction** (`Target -> Bid -> Strike -> Resolve`). The RPS **Clash** is a **deferred** pluggable layer (`future-possibilities.md`), no longer the core.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **The battle — hold / break / deal**                     | ✅ worked    | **Shape** (formations, the three ranks Vanguard / Outrider / Rearguard, crossing) is §4 below. **How the round resolves** is **[`combat.md`](combat.md)** — the shipped **eight-step round** (Havoc ... Advance), each step a declare/reveal wave, gated by `cargo test -p deckbound-board --test diagonal`. The earlier "five rounds on a fixed sub-phase schedule (Intercept / Volley / Raid / Clash / Breach), code pending" framing is **retired** (`retired-ideas.md`); §4's step-loop and schedule prose is superseded on that point (banner in §4). §4.3 actors-are-decks current — **attack derives from the strike card** (no strike card = Neither) |
...
> **RETIRED round structure and schedule (2026-07).** The round-loop and schedule that follow — the
> Marshal / Reveal / Ready / Engage / Refresh loop, the "five rounds" cap, and the
> Intercept / Volley / Raid / Clash / Breach sub-phase schedule (including the `Marshal` and
> `Sub-phase schedule` glossary terms below, which the **reference sample** still uses) — are **retired
> for the shipped product.** The product's round is the **eight-step schedule** (Havoc / Withdraw /
> Skirmish / Crossing / Defensive Volley / Raid / Assault / Advance), each step its own declare/reveal
> wave resolved on the spot, specified in **[`combat.md`](combat.md)** and gated by the diagonal test.
> The formation and rank *shape* above (§4.1–§4.5: front shields back, three declared ranks, crossing,
> groups) is current; only this round-loop and its sub-phase ordering are replaced. Kept here as design
> history and as the reference sample's model — do not implement the product from it.
> (Retired framing: `retired-ideas.md`.)
...
### 4.5 Groups — bind same-side Actors into one unit 🟡 *(phase model, 2026)*
