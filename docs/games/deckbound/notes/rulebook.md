...
- Each **character** has three zones: **Form** (who you are: health + gear + stats),
  **Potential** (what you can still play), **Active** (in play now). A card's
  **facing** tells you if it's working: face up = in effect, face down = spent.
...
**Health is cards.** Your Body is a little stack of plain cards; a hit **turns** them
face down (in Form). Lose your last Body card and you're **down** for the fight.
...
2. **Declare** (face-down, so it's hidden): your **stance** (Attack or Hold), your
   **target**, and your **Stance card** (see §4). You see everyone's *positions* — but not their
   stances — before choosing.
3. **Reveal** — flip everything at once.
4. **Resolve** — §5, then tidy up: clear scratches, refresh, return your guards' stances.

## 4. The stance — rock-paper-scissors of intent

Every clash, each side secretly picks one **stance**:
...
Win a stance and you **bank momentum** (Power/Speed/Precision cards) to spend later; **lose**
a stance (your intent gets countered) and you **forfeit your whole bank**. Build with
defense and Scheme, cash out with a Strike — gambling the pile each time.

**You only predict a foe you're engaging** (Holding, or trading attacks with them). Anyone
...
**out-predicting them** (a defense beats their Strike, a Strike spoils their Scheme). Some
...
**toughness**, turning Body cards face down *(appendix: the formula)*.
...
finesse** — one broad blow can't out-guess each foe; everyone predicting you answers it
their own way. To out-predict a *specific* foe, fight them one-on-one.
...
| -------------------- | ----------------------------------------------------------------------------------- |
| **Toughness**        | Health cards turned face down = ⌊ damage ÷ toughness ⌋; the Vitality card's "T"     |
| **Damage**           | (Power + banked Power) − armor[type], then ÷ toughness                              |
| **Stance cycle**     | Strike > Scheme > Defense > Strike; mirrors settled by tempo then Power             |
...
| ------------------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **Aldric** · Knight | Spd 4 · Pow 5 · Body 8 (T2) · Resolve 4           | Plate (armor: heat gets through), Shield (Block + Bash), **Bash** (fast blunt strike, staggers), the four Stance cards. *The wall — melee.*    |
| **Vera** · Duelist  | Spd 5 · Pow 3 · Body 4 (T1) · Resolve 2           | Blade (sharp), **Riposte** (Evade → counter → reposition, banks Speed), the four Stance cards. *The predictor — melee.*                        |
| **Sefa** · Mage     | Spd 2 · Pow 5 · Body 3 (T1) · Resolve 1 (fearful) | **Firestorm** (heat, hits 5 targets, once), **Frostbite** (cold, slows), the four Stance cards. *Glass cannon — ranged [2,2].*                 |
| **Bram** · Warden   | Spd 3 · Spr 5 · Body 5 (T2) · Resolve 4           | **Rally** (party-wide Resolve), **Dread** (fear attack), **Steel** (recover your nerve), the four Stance cards. *Spirit — ranged/inner [2,2].* |

**The warband**

| Foe          | Stats                                                           | Behavior                                                                                  |
| ------------ | --------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **Ironclad** | Spd 2 · Pow 6 · Body 8 (T3) · plate (heat gets through) · melee | *bluffs* (a hidden Stance deck); only heat cracks it, its blow is lethal unless predicted |
...
- **The two-stat clash reads cleanly** because each has one job ("Speed = who's first,
  ...
  "drop them first or out-predict them," not a separate Power rule.
...
- **Prediction + bandwidth + free hits** is the densest spot. It only stays manageable via the
  one-line rule "you predict only whom you can watch; the rest hit free." If a reader trips
