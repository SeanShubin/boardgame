# Bulwark vs Maul

## Bulwark → Maul   (never (∞))

```
Bulwark   slash-3 speed-2
Maul      health-4 toughness-4 armor-mail
start     [ ][ ][ ][ ]
round 1
  action 1  slash-3 ×½ = damage-1   acc 0+1=1 / toughness-4  no flip   [ ][ ][ ][ ]
  action 2  slash-3 ×½ = damage-1   acc 1+1=2 / toughness-4  no flip   [ ][ ][ ][ ]
  -- end round 1: acc 2 WASTED (round reset)
  -- walled: no path to a kill
```

## Maul → Bulwark   (kills in 3 round(s))

```
Maul      crush-6 speed-2
Bulwark   health-5 toughness-6 armor-plate
start     [ ][ ][ ][ ][ ]
round 1
  action 1  crush-6 ×2 = damage-12   acc 0+12=12 / toughness-6  FLIP (waste-6)   [#][ ][ ][ ][ ]
  action 2  crush-6 ×2 = damage-12   acc 0+12=12 / toughness-6  FLIP (waste-6)   [#][#][ ][ ][ ]
  -- end round 1: acc clear
round 2
  action 1  crush-6 ×2 = damage-12   acc 0+12=12 / toughness-6  FLIP (waste-6)   [#][#][#][ ][ ]
  action 2  crush-6 ×2 = damage-12   acc 0+12=12 / toughness-6  FLIP (waste-6)   [#][#][#][#][ ]
  -- end round 2: acc clear
round 3
  action 1  crush-6 ×2 = damage-12   acc 0+12=12 / toughness-6  FLIP (waste-6)   [#][#][#][#][#]
```

## Verdict

**Maul** wins — kills in 3 vs ∞.
