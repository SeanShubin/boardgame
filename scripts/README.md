...
| `run-debug`        | Run from a clean slate WITH the doom oracle on: delete the persisted save first (what "Start Over" does), then set `BOARDGAME_ORACLE` so the System deck's "Doom oracle" toggle opens on (combat foresight badges visible). Leaves any `.bak` backups alone. |
...
| `selection-states` | Show the four selection states (background / in-selection / completing / selectable), drawn by the real combat tile code.                                                                                                                                    |
