...
# pure rules::combat model (the round-sequence model: two declare waves - acts, then catches - resolved
# through the Inner/Crossing/Outer rings). Pick an option; the board updates; each option shows the
# solver's verdict and how many lines win vs lose. Mirrors the on-screen state to fight-screen.txt, and
# the WHOLE running log to fight-log.txt, in the repo root.
#
...
#   scripts\combat.ps1 3          # solo #3 (The Brood), fielded by its counter (Bastion)
#   scripts\combat.ps1 3 Raider   # solo #3, fielded by the Raider instead
#   scripts\combat.ps1 7          # The Hollow Rampart: raid past the Walls to the Sniper
#
